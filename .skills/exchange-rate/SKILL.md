...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-cbv1cssxetxd-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/{base_currency}` |
...
async function getExchangeRates(baseCurrency: string): Promise<ExchangeRateResponse> {
  ...
  const url = `https://app-cbv1cssxetxd-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/${baseCurrency.toUpperCase()}`;
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-cbv1cssxetxd-api-w9Rbo8E7p2b9.gateway.appmedo.com/v6/latest/${baseCurrency.toUpperCase()}`,
    {
      method: "GET",
      headers: {
        "Accept": "application/json",
