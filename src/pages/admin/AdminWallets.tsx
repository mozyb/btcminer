...
interface WalletSummary { currency: string; total_balance: number; user_count: number; }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        ...
        for (const w of data) {
          const cur = w.currency as string;
          if (!map[cur]) map[cur] = { total: 0, count: 0 };
          map[cur].total += Number(w.balance) || 0;
          map[cur].count += 1;
        }
        setWalletSummaries(Object.entries(map).map(([currency, v]) => ({
          currency, total_balance: v.total, user_count: v.count,
        })));
      }
      setLoading(false);
    })();
              ...
              <table className="w-full min-w-max text-sm">
                <thead>
                  <tr className="border-b border-border">
                    {["Currency", "Total User Balance", "Active Wallets"].map(h => (
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={3} className="py-8 text-center text-muted-foreground text-sm">No user wallets found</td></tr>
                    ...
                    <tr key={w.currency} className="border-b border-border last:border-0">
                      ...
                      <td className="py-3 px-3 font-mono text-primary whitespace-nowrap">{w.total_balance.toFixed(8)}</td>
                      <td className="py-3 px-3 font-mono text-foreground whitespace-nowrap">{w.user_count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
