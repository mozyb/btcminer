...
export default function CalculatorPage() {
  ...
  const btcPrice     = btc.btcPrice || 63000;

  // Recommended contracts from mock data based on hashrate entered
  const recommended = miningContracts
    .filter((c: { hashrate: number; hashrateUnit: string }) => {
      const normalised = c.hashrateUnit === "PH/s" ? c.hashrate * 1000 : c.hashrateUnit === "GH/s" ? c.hashrate / 1000 : c.hashrate;
        ...
        <section className="border-t border-border bg-card">
          <div className="max-w-7xl mx-auto px-4 md:px-6 py-10">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {recommended.map((c: { id: string; slug: string; name: string; hashrate: number; hashrateUnit: string; duration: number; totalPrice: number }) => {
                const normalised = c.hashrateUnit === "PH/s" ? c.hashrate * 1000 : c.hashrateUnit === "GH/s" ? c.hashrate / 1000 : c.hashrate;
                ...
                return (
                  <Card key={c.id} className="bg-card border-border h-full flex flex-col hover:border-primary/40 transition-colors">
                    <CardContent className="p-4 flex flex-col flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mb-3">{c.hashrate} {c.hashrateUnit} · {c.duration} days</p>
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        <p className="font-bold font-mono text-primary">${c.totalPrice.toLocaleString()}</p>
