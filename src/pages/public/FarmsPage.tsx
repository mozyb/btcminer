...
export default function FarmsPage() {
  ...
  const totalBtcMined = miningFarms.reduce((s, f) => s + f.totalBtcMined, 0);
  const totalMiners   = miningFarms.reduce((s, f) => s + f.activeMiners, 0);
  ...
  const localBusinessSchemas = miningFarms.map(f => ({
    ...
    "description": `${f.capacity} MW SHA-256 Bitcoin mining facility powered by ${f.powerSource}`,
  }));
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
              ...
              <div className="mt-4 border border-primary/30 bg-primary/5 rounded-lg p-4 flex flex-col md:flex-row gap-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 flex-1 text-xs">
                  {[
                    ...
                    { l: "Active Miners", v: farm.activeMiners.toLocaleString() },
                    { l: "BTC Mined", v: `${farm.totalBtcMined.toLocaleString()} BTC` },
                    { l: "Energy Source", v: farm.powerSource },
        ...
        <div className="mb-14">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {miningFarms.map(farm => {
              const meta = FARM_META[farm.id];
              const offline = farm.activeMiners - farm.onlineMiners;
              const uptimePct = (farm.onlineMiners / farm.activeMiners) * 100;
              return (
                <Card key={farm.id} className="bg-card border-border overflow-hidden h-full flex flex-col">
                  ...
                  <CardContent className="p-5 flex flex-col flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      {[
                        { icon: Zap,         label: "Power Source",      value: farm.powerSource },
                    ...
                    <div className="border-t border-border pt-4 mt-auto">
                      ...
                      <div className="grid grid-cols-3 gap-3 text-center text-xs">
                        <div>
                          ...
                          <p className="font-mono font-bold text-foreground">{farm.activeMiners.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Online</p>
                          <p className="font-mono font-bold text-success">{farm.onlineMiners.toLocaleString()}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">BTC Mined</p>
                          <p className="font-mono font-bold text-primary">{farm.totalBtcMined.toLocaleString()}</p>
                        </div>
                      </div>
                      {offline > 0 && (
