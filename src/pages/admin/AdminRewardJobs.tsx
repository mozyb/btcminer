...
export default function AdminRewardJobs() {
  ...
  const triggerJob = useCallback(async (runDate?: string) => {
    ...
    try {
      ...
      const result = data as { success: boolean; message?: string; error?: string; contracts_processed?: number; total_btc_credited?: number; rewardsDistributed?: number };
      if (!result.success) throw new Error(result.error ?? "Unknown error");
      toast.success(
        result.message ??
        `Job complete — ${result.contracts_processed ?? 0} contracts, ${fmtBtc(result.total_btc_credited ?? result.rewardsDistributed ?? 0)} BTC credited`
      );
      await loadJobs();
    } catch (err) {
