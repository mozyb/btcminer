...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-cbv1cssxetxd-api-pLVzAEz1ZQOL.gateway.appmedo.com/v1/videos/omni-video/{task_id}` |
| HTTP Method | `GET` |
...
async function queryOmniVideoTask(taskId: string): Promise<OmniVideoQueryResult> {
  const response = await fetch(
    `https://app-cbv1cssxetxd-api-pLVzAEz1ZQOL.gateway.appmedo.com/v1/videos/omni-video/${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
