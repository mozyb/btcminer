  ...
  try {
    ...
    if (provider) {
      try {
        ...
        } else if (provider.provider_type === "resend" && (provider.api_key || Deno.env.get("RESEND_API_KEY"))) {
          const resendKey = provider.api_key || Deno.env.get("RESEND_API_KEY")!;
          const rRes = await fetch("https://api.resend.com/emails", {
            method: "POST",
            headers: { "Authorization": `Bearer ${resendKey}`, "Content-Type": "application/json" },
            body: JSON.stringify({
              from: `${provider.from_name ?? "BTCMiner.online"} <${provider.from_email ?? "noreply@btcminer.online"}>`,
