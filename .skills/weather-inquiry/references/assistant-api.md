...
**Endpoint**: `POST https://app-cbv1cssxetxd-api-79jKPlpvAJ0L.gateway.appmedo.com/assistant/session`
...
**Endpoint**: `POST https://app-cbv1cssxetxd-api-oYA6ZxVqyK8a.gateway.appmedo.com/assistant/session/{session_id}`
...
async function startAssistantSession(prompt: string): Promise<AssistantResponse> {
  const response = await fetch(
    "https://app-cbv1cssxetxd-api-79jKPlpvAJ0L.gateway.appmedo.com/assistant/session",
    {
      method: "POST",
      headers: {
...
async function resumeAssistantSession(
  ...
  const response = await fetch(
    `https://app-cbv1cssxetxd-api-oYA6ZxVqyK8a.gateway.appmedo.com/assistant/session/${sessionId}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
