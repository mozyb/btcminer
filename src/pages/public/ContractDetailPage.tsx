...
=======
>>>>>>> 72e4db5c47454ac327e75930f78f07f22a878079
...
export default function ContractDetailPage() {
  ...
  useEffect(() => {
    getPublicContractTemplates()
      .then(data => { setContracts(data); setLoaded(true); })
      .catch(() => setLoaded(true));
  }, []);

  const contract = contracts.find(c => c.slug === slug);
  const related  = contracts.filter(c => c.slug !== slug && c.algorithm === contract?.algorithm).slice(0, 3);

  useEffect(() => {
    if (loaded && !contract) navigate("/marketplace", { replace: true });
  }, [loaded, contract, navigate]);

  if (!loaded || !contract) return null;

  const displayPrice = contract.promotional_price ?? contract.price;
  const networkTH    = btc.networkHashrate > 0 ? btc.networkHashrate * 1e6 : 850e6;
  const hashrateInTH = contract.hashrate_unit === "PH/s" ? contract.hashrate * 1000
                     : contract.hashrate_unit === "GH/s" ? contract.hashrate / 1000
  ...
  const productSchema = {
    ...
    description: `Purchase ${contract.hashrate} ${contract.hashrate_unit} of ${contract.algorithm} mining hashrate for ${contract.duration} days. Earn daily Bitcoin mining rewards from our enterprise data centers.`,
    brand: { "@type": "Brand", name: "BTCMiner.online" },
    offers: {
      "@type": "Offer",
      price: displayPrice,
      priceCurrency: contract.currency ?? "USD",
      availability: contract.remaining_capacity > 0 ? "https://schema.org/InStock" : "https://schema.org/OutOfStock",
    ...
    mainEntity: [
      {
        ...
        acceptedAnswer: { "@type": "Answer", text: `The ${contract.name} provides ${contract.hashrate} ${contract.hashrate_unit} of ${contract.algorithm} mining power for ${contract.duration} days, running on ${contract.hardware} hardware in our enterprise data centers.` },
      },
      {
        "@type": "Question",
  ...
  const contractFaq = [
    { q: `What is the ${contract.name} contract?`, a: `The ${contract.name} provides ${contract.hashrate} ${contract.hashrate_unit} of ${contract.algorithm} mining power for ${contract.duration} days on ${contract.hardware} hardware in our data centers.` },
    ...
    { q: "What are the maintenance fees?", a: `The daily maintenance fee is $${contract.maintenance_fee} per TH/s per day. This covers electricity, cooling, and hardware operations. Fees are deducted from gross mining rewards before crediting your wallet.` },
      ...
      <div className="bg-card border-b border-border">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            {contract.remaining_capacity > 0
              ? <Badge className="bg-success/10 text-success border-success/20">{contract.remaining_capacity} slots available</Badge>
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-foreground text-balance mb-3">
            {contract.name} — {contract.hashrate} {contract.hashrate_unit} Bitcoin Mining Contract
          </h1>
          <p className="text-muted-foreground text-lg max-w-2xl text-pretty">
            Purchase {contract.hashrate} {contract.hashrate_unit} of {contract.algorithm} mining hashrate for {contract.is_lifetime ? "a lifetime" : `${contract.duration} days`}.
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { icon: Zap,       label: "Hashrate",     val: `${contract.hashrate} ${contract.hashrate_unit}` },
                { icon: Clock,     label: "Duration",     val: contract.is_lifetime ? "Lifetime" : `${contract.duration} Days` },
            ...
            <Card className="bg-card border-border">
              <CardContent className="p-6">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {[
                    { label: "Contract Cost",   val: `$${displayPrice.toLocaleString()}`, sub: "One-time purchase" },
            ...
            <div className="sticky top-24">
              <Card className="bg-card border-primary/30">
                <CardContent className="p-6">
                  ...
                  {contract.discount_price && <p className="text-xs line-through text-muted-foreground">${contract.discount_price.toLocaleString()}</p>}
                  <p className="text-3xl font-bold font-mono text-primary mb-1">${displayPrice.toLocaleString()}</p>
                  <p className="text-xs text-muted-foreground mb-5">{contract.currency ?? "USD"}</p>
                  ...
                  <div className="space-y-2 mb-5 text-sm">
                    {[
                      `${contract.hashrate} ${contract.hashrate_unit} hashrate`,
                      contract.is_lifetime ? "Lifetime contract" : `${contract.duration}-day contract`,
                      `$${contract.maintenance_fee}/TH/day maintenance`,
        ...
        {related.length > 0 && (
          <div className="mt-16">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {related.map(r => {
                const rHashrateTH = r.hashrate_unit === "PH/s" ? r.hashrate * 1000
                                  : r.hashrate_unit === "GH/s" ? r.hashrate / 1000
                ...
                return (
                  <Card key={r.id} className="bg-card border-border h-full flex flex-col hover:border-primary/40 transition-colors">
                    <CardContent className="p-5 flex flex-col flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mb-3">{r.hashrate} {r.hashrate_unit} · {r.is_lifetime ? "Lifetime" : `${r.duration} days`}</p>
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        <p className="font-bold font-mono text-primary">${rPrice.toLocaleString()}</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-card border-t border-border p-4">
        <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
          <div>
            ...
            <p className="font-bold font-mono text-primary">${displayPrice.toLocaleString()}</p>
          </div>
