...
import { getPaymentMethods, getUserExternalWallets, submitManualTransaction, notifyAdmin, notifyUser, type PaymentMethod, type UserExternalWallet } from "@/lib/api";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await notifyAdmin({
        ...
        currency:     method.currency,
        network:      ((method as unknown) as Record<string, unknown>)["network"] ?? method.currency,
        amount:       n,
        usd_value:    usdValue(),
        transaction_id: txId,
        wallet_address: ((method as unknown) as Record<string, unknown>)["deposit_address"] ?? method.admin_address ?? "",
        status:       "Pending",
      });
      // Notify user their deposit was received
      notifyUser({
