...
const PLANS = [
  {
    ...
    features: { dailyPayouts: true, monitoring: true, extension: true, prioritySupport: false, poolChoice: false, manager: false },
  },
  {
    id: "pro",
    ...
    features: { dailyPayouts: true, monitoring: true, extension: true, prioritySupport: true, poolChoice: false, manager: false },
  },
  {
    id: "enterprise",
    ...
    duration: 365,
    price: 7200,
    maintenanceFee: 0.0022,
    badge: "Best Value",
    badgeColor: "bg-success/20 text-success border-success/30",
    risk: "Moderate",
    riskColor: "text-warning",
    features: { dailyPayouts: true, monitoring: true, extension: true, prioritySupport: true, poolChoice: true, manager: true },
  },
];

const COMPARE_ROWS = [
  { key: "hashrate",     label: "Hashrate",         format: (p: typeof PLANS[0]) => `${p.hashrate.toLocaleString()} TH/s` },
  { key: "duration",     label: "Duration",          format: (p: typeof PLANS[0]) => `${p.duration} days` },
  { key: "price",        label: "Contract Price",    format: (p: typeof PLANS[0]) => `$${p.price.toLocaleString()}` },
  { key: "maintenance",  label: "Maintenance Fee",   format: (p: typeof PLANS[0]) => `$${p.maintenanceFee}/TH/d` },
];

const FEATURE_LABELS: { key: keyof typeof PLANS[0]["features"]; label: string }[] = [
...
export default function PricingPage() {
  ...
  const sortedPlans = [...PLANS].sort((a, b) => {
    ...
    const map: Record<string, number> = {
      ...
      maintenance: a.maintenanceFee - b.maintenanceFee,
    };
    return sortDir === "asc" ? (map[sortKey] ?? 0) : -(map[sortKey] ?? 0);
  });

  // Featured marketplace contracts (SHA-256 only)
  const featuredContracts = miningContracts.filter(c => c.algorithm === "SHA-256").slice(0, 4);
  ...
  const productSchemas = PLANS.map(p => ({
    ...
    "@type": "Product",
    "name": p.name,
    "description": `${p.hashrate} TH/s SHA-256 Bitcoin mining contract for ${p.duration} days with daily payouts and transparent fees.`,
    "offers": {
      "@type": "Offer",
      "price": p.price,
      "priceCurrency": "USD",
      "availability": "https://schema.org/InStock",
      "url": "https://btcminer.online/pricing",
    },
  }));
    ...
    <PublicLayout>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-5 mb-12">
          {PLANS.map(plan => {
            const daily = dailyBTC(plan.hashrate);
            const dailyUsd = daily * btc.btcPrice;
            const totalBtcEst = totalBTC(plan.hashrate, plan.duration);
            const totalUsdEst = totalUSD(plan.hashrate, plan.duration);
            const maintTotal = plan.maintenanceFee * plan.hashrate * plan.duration;
            const roiPct = plan.price > 0 ? ((totalUsdEst - maintTotal - plan.price) / plan.price) * 100 : 0;
            const roiProgress = Math.min(Math.max((totalUsdEst / plan.price) * 100, 0), 150);

            const isPro = plan.id === "pro";

            return (
              <Card key={plan.id} className={`h-full flex flex-col bg-card ${isPro ? "border-primary/50 ring-2 ring-primary/20" : "border-border"}`}>
                {isPro && (
                ...
                <CardContent className="p-5 md:p-6 flex flex-col flex-1">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="font-bold text-lg text-foreground text-balance">{plan.name}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5">SHA-256 · Bitcoin Network</p>
                    </div>
                    <Badge className={`text-xs shrink-0 ${plan.badgeColor}`}>{plan.badge}</Badge>
                  </div>

                  {/* Price */}
                  <div className="mb-4">
                    <span className="text-3xl font-bold font-mono text-primary">${plan.price.toLocaleString()}</span>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4 text-sm">
                    {[
                      { label: "Hashrate", value: `${plan.hashrate.toLocaleString()} TH/s` },
                      { label: "Duration", value: `${plan.duration} days` },
                      { label: "Algorithm", value: "SHA-256" },
                      { label: "Maintenance", value: `$${plan.maintenanceFee}/TH/d` },
                    ].map(s => (
                  ...
                  {btc.loading ? (
                    ...
                    <div className="mb-4 p-3 bg-success/5 border border-success/20 rounded">
                      ...
                      <p className="font-mono text-xs text-foreground font-semibold mt-0.5">
                        ≈ ${totalUsdEst.toLocaleString("en-US", { maximumFractionDigits: 0 })} over {plan.duration}d
                      </p>
                    </div>
                  )}
                  ...
                  <div className="mb-5 p-3 border border-border rounded bg-muted/20">
                    ...
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-muted-foreground">Investment: <span className="font-mono text-foreground">${plan.price.toLocaleString()}</span></span>
                      <span className={plan.riskColor + " font-semibold"}>Risk: {plan.risk}</span>
                    </div>
                  ...
                  <div className="space-y-1.5 mb-6 flex-1">
                    {FEATURE_LABELS.map(f => (
                      <div key={f.key} className={`flex items-center gap-2 text-xs ${plan.features[f.key] ? "text-muted-foreground" : "text-muted-foreground/40"}`}>
                        <CheckCircle className={`w-3.5 h-3.5 shrink-0 ${plan.features[f.key] ? "text-success" : "text-muted-foreground/30"}`} />
                        {f.label}
                      </div>
                    ))}
                  </div>
            ...
            <table className="w-full min-w-max text-sm border-collapse">
              <thead>
                <tr className="border-b border-border">
                  ...
                  {sortedPlans.map(p => (
                    <th key={p.id} className={`py-3 px-4 text-center whitespace-nowrap ${p.id === "pro" ? "bg-primary/5" : ""}`}>
                      <div className="font-bold text-foreground">{p.name}</div>
                      <Badge className={`text-[10px] mt-1 ${p.badgeColor}`}>{p.badge}</Badge>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {COMPARE_ROWS.map((row, i) => (
                  <tr key={row.key} className={`border-t border-border ${i % 2 === 1 ? "bg-muted/20" : ""}`}>
                    ...
                    {sortedPlans.map(p => (
                      <td key={p.id} className={`py-2.5 px-4 text-center font-mono whitespace-nowrap ${p.id === "pro" ? "bg-primary/5 font-bold text-primary" : "text-foreground"}`}>
                        {row.format(p)}
                      </td>
                    ))}
                  </tr>
                ...
                <tr className="border-t border-border">
                  ...
                  {sortedPlans.map(p => (
                    <td key={p.id} className={`py-2.5 px-4 text-center whitespace-nowrap ${p.id === "pro" ? "bg-primary/5" : ""}`}>
                      {btc.loading ? <Skeleton className="h-4 w-20 bg-muted mx-auto" />
                        : <span className="font-mono text-success font-bold text-xs">{dailyBTC(p.hashrate).toFixed(6)} ₿</span>}
                    </td>
                  ))}
                </tr>
                <tr className="border-t border-border bg-muted/20">
                  ...
                  {sortedPlans.map(p => (
                    <td key={p.id} className={`py-2.5 px-4 text-center whitespace-nowrap ${p.id === "pro" ? "bg-primary/5" : ""}`}>
                      {btc.loading ? <Skeleton className="h-4 w-20 bg-muted mx-auto" />
                        : <span className="font-mono font-bold text-foreground text-xs">${totalUSD(p.hashrate, p.duration).toLocaleString("en-US", { maximumFractionDigits: 0 })}</span>}
                    </td>
                  ))}
                </tr>
                {/* Feature rows */}
                ...
                <tr className="border-t border-border">
                  ...
                  {sortedPlans.map(p => (
                    <td key={p.id} className={`py-4 px-4 text-center whitespace-nowrap ${p.id === "pro" ? "bg-primary/5" : ""}`}>
                  ...
                  return (
                    <tr key={c.id} className="border-t border-border hover:bg-muted/20 transition-colors">
                      <td className="py-3 px-4 whitespace-nowrap">
                        ...
                        {c.badge && <Badge className="text-[10px] mt-0.5 bg-primary/10 text-primary">{c.badge.replace("_", " ")}</Badge>}
                      </td>
                      <td className="py-3 px-4 whitespace-nowrap"><span className="font-mono text-xs">{c.algorithm}</span></td>
                      <td className="py-3 px-4 whitespace-nowrap font-mono">{c.hashrate} {c.hashrateUnit}</td>
                      <td className="py-3 px-4 whitespace-nowrap font-mono">{c.duration}d</td>
                      <td className="py-3 px-4 whitespace-nowrap font-mono font-bold text-primary">${c.totalPrice.toLocaleString()}</td>
                      <td className="py-3 px-4 whitespace-nowrap">
                        {btc.loading
