...
import { CheckCircle, XCircle, Eye, MapPin, FileText, Clock, RefreshCw, ExternalLink, AlertTriangle } from "lucide-react";
...
export default function AdminKYC() {
  ...
  // Track images that failed to load (bucket deleted / file missing)
  const [brokenImgs, setBrokenImgs] = useState<Set<string>>(new Set());
  const markBroken = (url: string) => setBrokenImgs(prev => new Set(prev).add(url));
                  ...
                  <div key={doc.label} className="space-y-1.5">
                    ...
                    {doc.url ? (
                      <div className="border border-border rounded overflow-hidden bg-muted/20">
                        {brokenImgs.has(doc.url) ? (
                          /* File missing / bucket was recreated — show friendly fallback */
                          <div className="h-40 flex flex-col items-center justify-center gap-2 px-3 text-center">
                            <AlertTriangle className="w-7 h-7 text-warning opacity-70" />
                            <p className="text-xs text-muted-foreground leading-relaxed">
                              File unavailable.<br />Ask user to resubmit KYC.
                            </p>
                          </div>
                        ) : doc.url.match(/\.(jpg|jpeg|png|gif|webp)(\?.*)?$/i) || doc.url.includes("/object/public/") ? (
                          <img
                            src={doc.url}
                            alt={doc.label}
                            className="w-full h-40 object-cover"
                            onError={() => markBroken(doc.url!)}
                          />
                        ) : (
