...
| Item | Details |
|------|---------|
| Endpoint | `GET https://app-cbv1cssxetxd-api-oYA6Z8wDBRDa.gateway.appmedo.com/v1/videos/image2video/{id}` |
...
async function queryImageToVideoTask(taskId: string): Promise<QueryResponse> {
  const response = await fetch(
    `https://app-cbv1cssxetxd-api-oYA6Z8wDBRDa.gateway.appmedo.com/v1/videos/image2video/${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
